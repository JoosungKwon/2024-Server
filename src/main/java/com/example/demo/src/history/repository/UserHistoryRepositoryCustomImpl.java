...
import static com.example.demo.src.history.entity.QUserHistory.userHistory;
...
public class UserHistoryRepositoryCustomImpl implements UserHistoryRepositoryCustom {
    ...
    @Override
    public Page<UserHistory> findAllBy(AdminHistorySearchReq adminHistorySearchReq, Pageable pageable) {
        ...
        List<UserHistory> result = queryFactory.selectFrom(userHistory)
    ...
    private Predicate conditionDateStart(LocalDateTime start) {
        return userHistory.createdAt.goe(start);
    }


    private Predicate conditionDateEnd(LocalDateTime end) {
        return userHistory.createdAt.loe(end);
    }


    private Predicate conditionDomainId(Long domainId) {
        if (domainId != null) {
            return userHistory.id.eq(domainId);
        }
        return null;
    }
